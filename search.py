...
def maintraverse(fringe, problem):
    def pathfinder(v):
        path = []
        for item in v:
            path.append(item[1])
    ...
    while not fringe.isEmpty():
        ...
        if problem.isGoalState(s[-1][0]):
            return pathfinder(s[1:])
        if s[-1][0] not in visitedset:
            visitedset.add(s[-1][0])
            success = problem.getSuccessors(s[-1][0])
            for child in success:
                fringe.push(s + [(child[0], child[1], child[2] + s[-1][2])])
...
def depthFirstSearch(problem):
    ...
    "*** YOUR CODE HERE ***"
    fringe = util.Stack()
    return maintraverse(fringe, problem)
